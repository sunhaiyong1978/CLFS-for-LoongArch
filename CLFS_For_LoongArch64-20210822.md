...
sudo dnf install @core @c-development rpm-build git python3-devel texinfo \
                 zlib-devel xz-lzma-compat gettext-devel perl-FindBin gdbm-devel expat-devel \
...
　　**Linux-Firmware:** https://mirrors.edge.kernel.org/pub/linux/kernel/firmware/linux-firmware-20210716.tar.xz  
...
　　**ACPI-Update:** https://github.com/sunhaiyong1978/CLFS-for-LoongArch/releases/download/20210812/acpi-update-20210811.tar.gz  
...
pushd gcc
    git archive --format=tar --output ../gcc-12.0.0.tar "loongarch-12"
popd
mkdir gcc-12.0.0
pushd gcc-12.0.0
...
pushd ${BUILDDIR}/gcc-12.0.0
	patch -Np1 -i ${DOWNLOADDIR}/gcc-12-loongarch-fix-ldso_name.patch
	mkdir build
	pushd build
...
pushd ${BUILDDIR}/perl-5.34.0
    ...
    make
    make install
    mv -v ${SYSDIR}/cross-tools/bin/{,${CROSS_TARGET}-}perl
popd
```
...
pushd ${BUILDDIR}/Python-3.9.6
	...
	make
	make install
	for i in python3 python3-config
	do
        mv -v ${SYSDIR}/cross-tools/bin/{,${CROSS_TARGET}-}$i
    done
popd
```
...
pushd ${BUILDDIR}/cmake-3.21.1
    mkdir build
    pushd build
        LDFLAGS="-Wl,-rpath-link=${SYSDIR}/sysroot/usr/lib64" \
...
tar xvf ${DOWNLOADDIR}/Python-3.9.6.tgz -C ${BUILDDIR}
pushd ${BUILDDIR}/Python-3.9.6
	LIBS="-lz -lmpfr -lgmp" ./configure --prefix=/usr  --libdir=/usr/lib64 \
...
pushd ${BUILDDIR}/pip-21.2.4
    ${CROSS_TARGET}-python3 setup.py build
    ${CROSS_TARGET}-python3 setup.py install --prefix=${SYSDIR}/sysroot/usr
popd
```
...
tar xvf ${DOWNLOADDIR}/ninja-1.10.2.tar.gz -C ${BUILDDIR}
pushd ${BUILDDIR}/ninja-1.10.2
    CXX="${CROSS_TARGET}-g++" AR="${CROSS_TARGET}-ar" ${CROSS_TARGET}-python3 configure.py
    ninja
...
pushd ${BUILDDIR}/links-2.23
    CC="${CROSS_TARGET}-gcc" LIBS="-lpthread -lz -ldl" \
    ./configure --prefix=/usr --libdir=/usr/lib64 \
	            --build=${CROSS_HOST} --host=${CROSS_TARGET}
    make
  	make DESTDIR=${SYSDIR}/sysroot install
...
pushd ${BUILDDIR}/xfsprogs-5.12.0
    ...
    sed -i "/LIBBLKID =/s@-lblkid@${SYSDIR}/sysroot/usr/lib64/libblkid.so@g" include/builddefs
    make
    make DESTDIR=${SYSDIR}/sysroot install
popd
```
...
pushd ${BUILDDIR}/nss-3.69/nss
    ...
    make NATIVE_CC="gcc" CC="${CROSS_TARGET}-gcc" CCC="${CROSS_TARGET}-g++" \
         ...
         NSPR_INCLUDE_DIR=${SYSDIR}/sysroot/usr/include/nspr OS_TEST="loongarch64"
...
pushd ${BUILDDIR}/libxml2-2.9.12
    ...
    ./configure --prefix=/usr --libdir=/usr/lib64 --build=${CROSS_HOST} \
                ...
                --with-python=${SYSDIR}/cross-tools/bin/${CROSS_TARGET}-python3
...
pushd ${BUILDDIR}/libxslt-1.1.34
    ...
    ./configure --prefix=/usr --libdir=/usr/lib64 --build=${CROSS_HOST} \
                ...
                --with-libxml-include-prefix=${SYSDIR}/sysroot/usr/include/libxml2
    make
    make DESTDIR=${SYSDIR}/sysroot install
popd
```
...
pushd ${BUILDDIR}/zip30
    ...
    make prefix=${SYSDIR}/sysroot/usr MANDIR=${SYSDIR}/sysroot/usr/share/man/man1 -f unix/Makefile install
popd
```
...
pushd ${BUILDDIR}/unzip60
    ...
    make -f unix/Makefile CC="${CROSS_TARGET}-gcc -DLARGE_FILE_SUPPORT -DUNICODE_WCHAR -DUNICODE_SUPPORT" generic
    make prefix=${SYSDIR}/sysroot/usr MANDIR=${SYSDIR}/sysroot/usr/share/man/man1 -f unix/Makefile install
popd
```
...
```sh
tar xvf ${DOWNLOADDIR}/acpi-update-20210811.tar.gz
pushd acpi-update-20210811
...
#### Linux-Firmware
```sh
tar xvf ${DOWNLOADDIR}/linux-firmware-20210716.tar.xz -C ${BUILDDIR}
pushd ${BUILDDIR}/linux-firmware-20210716
	make DESTDIR=${SYSDIR}/sysroot install
popd
```
...
pushd ${SYSDIR}/sysroot
	sudo tar --xattrs-include='*' --owner=root --group=root -cjpf \
			${SYSDIR}/loongarch64-clfs-system-5.0.tar.bz2 *
popd
```
...
```sh
pushd /tmp/liveusb
    sudo tar -xvpf ${SYSDIR}/loongarch64-clfs-system-5.0.tar.bz2
popd
```
